    ...
    if st.session_state["transcript_text"]:
        try:
            ...
            MEMORY_TRIGGERS = [
                ...
                "earlier question", "before this"
            ]

            def is_memory_question(text):
                return any(t in text.lower() for t in MEMORY_TRIGGERS)

            if is_memory_question(user_input) and st.session_state.conversation_memory:
            ...
            # Pydantic validation with confidence
            qa_validated = QAResponse(
                ...
                confidence=min(1.0, len(answer_text) / max(1, len(st.session_state["transcript_text"])))
            )

            # Format response like your desired style
    ...
    if st.button("Summarize Video"):
        if st.session_state["transcript_text"]:
            try:
                ...
                summary_validated = SummaryResponse(summary=summary_result["result"],                  confidence = min(1.0, len(answer_text) / max(1, len(st.session_state["transcript_text"])))
)
